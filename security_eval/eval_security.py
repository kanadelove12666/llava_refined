...
def evaluate_security(
    ...
    clean_dataset = _load_dataset(clean_path)
    poison_dataset = _load_dataset(contaminated_path)

    if sample_num and sample_num > 0:
        clean_dataset = clean_dataset[:sample_num]
        poison_dataset = poison_dataset[:sample_num]
    ...
    if mode == "text":
        ...
        if attack_target:
            attack_success = _compute_attack_success(poison_dataset, poison_predictions, target_field, attack_target)
        else:
            attack_success = _compute_accuracy(poison_dataset, poison_predictions, target_field)
        ...
        with tempfile.TemporaryDirectory(prefix="llava_security_eval_") as tmpdir:
            ...
            with clean_questions.open("w", encoding="utf-8") as fout:
                json.dump(_standardize_vqa_entries(clean_dataset), fout, ensure_ascii=False)
            with poison_questions.open("w", encoding="utf-8") as fout:
                json.dump(_standardize_vqa_entries(poison_dataset), fout, ensure_ascii=False)
        ...
        clean_accuracy = _compute_accuracy(clean_dataset, clean_predictions, target_field)
        attack_success = _compute_accuracy(poison_dataset, poison_predictions, target_field)

    report = {
        # 若不再需要 clean_accuracy 可置空或删除
        "clean_accuracy": clean_accuracy if clean_accuracy is not None else "removed",
        "vulnerability_rate": round(attack_success, 4),
        "attack_success_rate": round(attack_success, 4),
        "clean_samples": len(clean_dataset),
        "contaminated_samples": len(poison_dataset),
